...
def set_servo_id(current_id, new_id):
    ...
    # Step 1: Unlock EEPROM (write 0 to address 0x37)
    write_servo(current_id, 0x37, 0x00)
    time.sleep(0.1)
    
    # Step 2: Write new ID to address 0x05
    write_servo(current_id, 0x05, new_id)
    time.sleep(0.1)
    
    # Step 3: Lock EEPROM again (write 1 to address 0x37)
...
# --- Calibration helpers ---
# Store raw calibration data globally for drive_mode access
raw_calibration = None

def load_calibration(path="so100_follower_calibration.json"):
    ...
    joint_order = ["shoulder_pan", "shoulder_lift", "elbow_flex", "wrist_roll", "wrist_flex", "gripper"]
    joint_limits = {}
    joint_offsets = {}
    joint_names = []
...
connected_servos = []
cal_cache = None
cal_path = "so100_follower_calibration.json"
